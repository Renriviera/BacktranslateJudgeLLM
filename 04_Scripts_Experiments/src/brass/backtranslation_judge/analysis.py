...
def gold_map(path):
    ...
    for row in rows:
        ...
        if source not in ("human_adjudicated", "human_consensus", "human_review_pending"):
            raise ValueError(
                "Gold must identify human_adjudicated or human_consensus provenance; automated labels are not gold"
            )
        ...
        raters = row.get("rater_ids", [])
        if len(set(raters)) < 2 or any(not isinstance(r, str) or not r.strip() for r in raters):
            raise ValueError("Gold requires two distinct human raters")
        if source == "human_adjudicated":
            if not row.get("adjudicator_id") or row["adjudicator_id"] in raters:
                raise ValueError("Adjudicator must be distinct from initial raters")
        elif source == "human_review_pending":
            if row["status"] != "uncertain":
                raise ValueError("Pending adjudication cannot be resolved gold")
        else:
            ratings = row.get("individual_ratings", [])
            if len(ratings) != 2 or {r.get("rater_id") for r in ratings} != set(raters):
                raise ValueError("Consensus requires both original human ratings")
            if any(
                r.get("uncertain")
                or (r.get("assistance"), r.get("prohibited"))
                != (row.get("assistance"), row.get("prohibited"))
                for r in ratings
            ):
