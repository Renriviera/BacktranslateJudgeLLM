...
from brass.paths import hf_home, hf_hub_cache, model_snapshot
...
def initialize(repo, out):
    ...
    models = {
        ...
        "judge": dict(
            ...
            revision=QWEN_REV,
            snapshot=str(Path(os.environ["BACKTRANSLATION_QWEN_PATH"]).expanduser() if os.environ.get("BACKTRANSLATION_QWEN_PATH") else model_snapshot("Qwen/Qwen3-32B", QWEN_REV)),
        ),
    }
    code = {
        str(p.relative_to(repo)): file_digest(p)
        for p in sorted((repo / "04_Scripts_Experiments/src/brass/backtranslation_judge").rglob("*"))
        if p.suffix in (".py", ".json")
    }
    code["04_Scripts_Experiments/src/brass/serving/vllm_engine.py"] = file_digest(
        repo / "04_Scripts_Experiments/src/brass/serving/vllm_engine.py"
    )
    code["04_Scripts_Experiments/scripts/backtranslation_judge.py"] = file_digest(
        repo / "04_Scripts_Experiments/scripts/backtranslation_judge.py"
    )
    spec = dict(
        models=models,
...
def run(repo, out, limit=0, wait_pid=None):
    ...
    with (out / "orchestrator.lock").open("a") as lock:
        ...
        for name in STAGES:
            cmd = [
                sys.executable,
                str(repo / "04_Scripts_Experiments/scripts/backtranslation_judge.py"),
                "stage",
                "--run-dir",
                str(out),
                "--stage",
