...
from utils.opt_utils import get_latest_commit_info, load_model_and_tokenizer
...
if args.attacker == "AdvBench":
    with open('../datasets/harmful_behaviors_custom.json', encoding='utf-8') as file:
    ...
    with open('../datasets/custom_prompts.json', encoding='utf-8') as file:
        attack_prompts = json.load(file)
...
for prompt in tqdm(attack_prompts):
    ...
    is_list_input = isinstance(user_prompt, list)
    if is_list_input:
        user_prompts = user_prompt  # Multiple prompts
        logging.info(f"User Prompts (batch of {len(user_prompts)})")
        for idx, up in enumerate(user_prompts[:3]):  # Log first 3
            logging.info(f"  [{idx}]: \"{up[:100]}...\"")
    else:
        user_prompts = [user_prompt]  # Single prompt wrapped in list
        ...
        if args.is_defense:
            ...
            # Baseline Implementations
                ...
                input_manager = PromptManager(tokenizer=tokenizer,
                    ...
                    whitebox_attacker=whitebox_attacker)
                inputs = input_manager.get_inputs()
                ppl_score = ppl_calculator.get_perplexity(user_prompt_single)
            ...
            if "yes" in outputs_check.lower():
                ...
                logging.info("Self-Exam failed. Return original output.")
