...
load_dotenv()
from eval_utils_GPT import advbench_metric_test_prefixes, is_api_model_path
...
def evaluate_single_completion_hash_check(behavior_dict, completion):
    ...
    from eval_utils_GPT import check_output_with_sliding_window, compute_hashes_with_sliding_window
...
def evaluate_behavior_without_early_stopping(behavior_dict, data, cls, cls_params, use_advbench_first=True):
    ...
    if 'hash_check' in tags:
        ...
        from eval_utils_GPT import compute_results_advbench, compute_results_classifier
...
def load_completions(completions_path):
    ...
    if isinstance(data, dict) and 'metadata' in data and 'completions' in data:
        ...
        if metadata.get('max_length_regeneration_enabled'):
            print("Max length regeneration was enabled:")
...
def handle_null_generations(completions):
    ...
    if null_count > 0:
        ...
        print("These will be treated as empty strings (label=0)")
...
def main():
    ...
    if completion_metadata and completion_metadata.get('max_length_regeneration_enabled'):
        print("Max length regeneration was enabled during generation")
    ...
    print("All steps for all behaviors have been saved to the results file.")
    print("Each behavior is a key with a list containing: step, test_case, generation, label, cls_label.")
