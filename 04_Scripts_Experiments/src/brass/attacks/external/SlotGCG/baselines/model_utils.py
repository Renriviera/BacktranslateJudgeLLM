...
# BRASS: ray/vllm are only needed for the vLLM completion+eval steps, not the GCG optimizer.
# Keep them optional so step-1 (generate_test_cases) runs in the autograd-only attack venv.
try:
    import ray
...
def _get_fschat_conv(model_name_or_path=None, fschat_template=None, system_message=None, **kwargs):
    ...
    assert template and template.dict()['template_name'] != 'one_shot', f"Can't find fschat conversation template `{template_name}`. See for supported template"
