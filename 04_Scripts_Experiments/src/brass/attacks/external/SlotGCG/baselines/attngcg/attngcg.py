...
from .attngcg_utils import attention_loss, get_nonascii_toks, sample_control


class AttnGCG(SingleBehaviorRedTeamingMethod):
    def __init__(self,
                 target_model,
                 targets_path,
                 num_steps=50,
        ...
        self.attention_pooling_method = attention_pooling_method
        self.attention_weight = attention_weight
        self.attention_weight_dict = attention_weight_dict if attention_weight_dict else {"goal":0,"control":1}
    ...
    def generate_test_cases_single_behavior(self, behavior_dict, verbose=False, **kwargs):
        ...
        for i in tqdm(range(num_steps)):
            ...
            if self.use_prefix_cache:
                ...
                goal_slice = slice(0, behavior_embeds.shape[1])
            else:
                ...
                goal_slice = slice(before_embeds.shape[1], before_embeds.shape[1] + behavior_embeds.shape[1])

            control_slice = slice(goal_slice.stop, goal_slice.stop + optim_embeds.shape[1])
            target_slice = slice(control_slice.stop + after_embeds.shape[1], control_slice.stop + after_embeds.shape[1] + target_embeds.shape[1])
            ...
            slice_dict = {"goal":goal_slice,
                          "control":control_slice}

            attn_loss = attention_loss(attention=attention, slice_dict=slice_dict, attention_pooling_method=self.attention_pooling_method, attention_weight_dict=self.attention_weight_dict, target_start_idx=tmp)
            ...
            control_slice = slice(goal_slice.stop, goal_slice.stop + sampled_top_embeds.shape[1])

            slice_dict = {"goal":goal_slice,
                        "control":control_slice}

            loss, logit_losses, attn_losses = find_executable_batch_size(self.compute_candidates_loss, self.search_batch_size)(input_embeds, target_ids, slice_dict)
            ...
            del input_embeds, sampled_top_embeds, logits, shift_logits, logit_loss, attn_loss, attention, logit_losses, attn_losses
    ...
    def compute_candidates_loss(self, search_batch_size, input_embeds, target_ids, slice_dict):
        ...
        for i in range(0, input_embeds.shape[0], search_batch_size):
            ...
            for b_idx in range(last_layer_attention.size(0)):
                single_attention = last_layer_attention[b_idx:b_idx+1]
                one_sample_attn_loss = attention_loss(
                    single_attention,
                    slice_dict=slice_dict,
                    attention_pooling_method=self.attention_pooling_method,
                    attention_weight_dict=self.attention_weight_dict,
                    target_start_idx=tmp
                )
                all_attn_loss.append(one_sample_attn_loss)

            all_logit_loss.append(loss_logit)

            del outputs, logits, loss_logit, attentions, one_sample_attn_loss
            torch.cuda.empty_cache()
            gc.collect()
