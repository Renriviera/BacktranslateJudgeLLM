...
from .gcg_posinit_attention_ray_actors import (
...
class EnsembleGCG_posinit_attention(RedTeamingMethod):
    use_ray = True
    ...
    def generate_test_cases(self, behaviors, verbose=False):
        ...
        for i in tqdm(range(self.num_steps)):
            ...
            optim_str_list_ = [self.sample_tokenizer.decode([token_id]) for token_id in optim_ids[0]]


            # 스텝별로 저장 (inline JSON)
            step_info = {
                'step': i,
                'optim_str': optim_str_list_,
                'optim_pos': optim_pos.tolist()
            }

            # inline JSON으로 한 줄씩 추가 (append mode)
            with open("/home/work/kisti_2/Jailbreak/Attack/Harmbench/result_inline_ensemble/a.jsonl", 'a', encoding='utf-8') as f:
