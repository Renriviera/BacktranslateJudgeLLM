...
class GCGModelWorkerPosinit:
    def __init__(self,
                 ...
                 K=7,  # Add K parameter for multi-coordinate update
                 **model_kwargs):
        random.seed(seed)
    ...
    def compute_attention_scores_single(self, init_token_id, behavior_idx, behavior_length):
        ...
        # 모든 가능한 위치에 토큰 삽입 (behavior 시퀀스 길이 + 1개 위치)
        ...
        # Attention weights 추출 (상위 절반 레이어들의 합)
        attentions = torch.stack(outputs.attentions[upper_half_start:]).sum(dim=(0, 2))

        # 삽입된 토큰들의 실제 위치 계산 (삽입 후 위치가 shift됨)
        actual_positions = []
        for i, pos in enumerate(insert_positions):
            actual_pos = pos + i  # i번째 토큰까지 삽입되면서 위치가 shift됨
            actual_positions.append(actual_pos)

        # 크기 정보
        batch_size, query_len, key_len = attentions.shape
        cached_len = key_len - query_len if self.use_prefix_cache else 0  # prefix_cache 길이
        ...
        # 인덱스 설정 (Query 기준 - after 부분의 attention을 사용)
        if self.use_prefix_cache:
    ...
    def compute_token_gradient_posinit(self, worker_optim_ids, worker_optim_pos, selected_behavior_idx, max_behavior_length=None):
        ...
        behavior_with_optim = insert_optim_embed_pos(behavior_embeds, optim_embeds, scaled_optim_pos)
    ...
    def compute_loss_on_candidates_batch_posinit(self, sampled_top_indices, worker_optim_pos, selected_behavior_indices, max_behavior_length=None, current_optim_ids=None):
        """
        Compute loss for position-based candidates across multiple behaviors with multi-coordinate update
        Following the exact logic from the reference single behavior implementation
        """
        all_best_losses = []
        all_best_candidates = []

        for behavior_idx in selected_behavior_indices:
            # 먼저 모든 후보에 대한 손실 계산 (reference code와 동일)
            single_losses = self.compute_loss_on_candidates_posinit(
                sampled_top_indices, worker_optim_pos, behavior_idx, max_behavior_length
            )

            # Apply multi-coordinate update for this behavior (reference code 논리 그대로)
            best_candidate, best_loss = self.apply_multicoord_update_single_behavior(
                sampled_top_indices, worker_optim_pos, behavior_idx, single_losses,
                max_behavior_length, current_optim_ids
            )

            all_best_losses.append(best_loss)
            all_best_candidates.append(best_candidate)

        # 각 behavior에 대한 최적 후보의 손실들을 스택 (reference code와 동일한 구조)
        behavior_losses = torch.stack(all_best_losses, dim=0)
        return behavior_losses, all_best_candidates

    def apply_multicoord_update_single_behavior(self, sampled_top_indices, worker_optim_pos, behavior_idx, single_losses, max_behavior_length=None, current_optim_ids=None):
        """
        Apply multi-coordinate update for a single behavior following the reference implementation exactly
        """
        if current_optim_ids is None:
            # Fallback to best single candidate if no current optim_ids provided
            best_idx = single_losses.argmin()
            return sampled_top_indices[best_idx], single_losses[best_idx]

        # Sort losses and get top K candidates (reference code logic)
        k = min(self.K, len(single_losses))
        losses_temp, idx1 = torch.sort(single_losses, descending=False)

        # Convert candidates to text for multi-coordinate processing (exact same as reference)
        new_adv_suffix = self.tokenizer.batch_decode(sampled_top_indices, skip_special_tokens=True)
        adv_suffix = self.tokenizer.decode(current_optim_ids.squeeze(0), skip_special_tokens=True)

        # Apply manual multi-coordinate merging using utils function (exact same logic)
        try:
            all_new_adv_suffix = apply_manual_multicoord_update(
                self.tokenizer, new_adv_suffix, adv_suffix, idx1[:k]
            )

            if all_new_adv_suffix:
                # Convert back to control indices for evaluation (same as reference)
                multi_candidates_ids = []
                for suffix in all_new_adv_suffix:
                    suffix_ids = self.tokenizer(suffix, return_tensors="pt", add_special_tokens=False)['input_ids']
                    if suffix_ids.shape[1] == current_optim_ids.shape[1]:  # Same length check
                        multi_candidates_ids.append(suffix_ids)

                if multi_candidates_ids:
                    multi_candidates_ids = torch.cat(multi_candidates_ids, dim=0).to(self.device)

                    # Evaluate multi-coordinate candidates (same evaluation logic as reference)
                    multi_losses = self.compute_loss_on_candidates_posinit(
                        multi_candidates_ids, worker_optim_pos, behavior_idx, max_behavior_length
                    )

                    # Select best multi-coordinate candidate
                    best_multi_idx = multi_losses.argmin()
                    best_candidate = multi_candidates_ids[best_multi_idx]
                    best_loss = multi_losses[best_multi_idx]

                    print(f"Multi-coordinate update: improved loss from {single_losses.min().item():.4f} to {best_loss.item():.4f}")
                    return best_candidate, best_loss
                else:
                    print("No valid multi-coordinate candidates found, using single best")
                    best_idx = single_losses.argmin()
                    return sampled_top_indices[best_idx], single_losses[best_idx]
            else:
                print("Multi-coordinate update failed, using single best")
                best_idx = single_losses.argmin()
                return sampled_top_indices[best_idx], single_losses[best_idx]

        except Exception as e:
            print(f"Error in multi-coordinate update: {e}, falling back to single best")
            best_idx = single_losses.argmin()
            return sampled_top_indices[best_idx], single_losses[best_idx]
    ...
    def compute_loss_on_candidates_posinit_simple(self, sampled_top_indices, worker_optim_pos, selected_behavior_indices, max_behavior_length=None):
        """
        Compute simple average loss for position-based candidates across multiple behaviors.
        Returns average loss for each candidate across all selected behaviors.
        Similar to gcg_posinit_attention_ensemble's compute_loss_on_candidates_batch_posinit.
        """
        all_losses = []
        for behavior_idx in selected_behavior_indices:
            behavior_losses = self.compute_loss_on_candidates_posinit(
                sampled_top_indices, worker_optim_pos, behavior_idx, max_behavior_length
            )
            all_losses.append(behavior_losses)

        # Stack and average across behaviors: (num_behaviors, num_candidates) -> (num_candidates,)
        stacked_losses = torch.stack(all_losses, dim=0)  # Shape: (num_behaviors, num_candidates)
        avg_losses = stacked_losses.mean(dim=0)  # Average across behaviors: (num_candidates,)
