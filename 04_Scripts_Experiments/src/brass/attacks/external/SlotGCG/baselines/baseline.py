...
class SingleBehaviorRedTeamingMethod(RedTeamingMethod):
    ...
    def generate_test_cases(self, behaviors, verbose=False, save_dir=None, run_id=None):
        ...
        for behavior_dict in tqdm(behaviors, total=len(behaviors)):
            ...
            # BRASS patch: persist each behavior immediately so a crash on a multi-day run only
            # loses the in-progress behavior (a later restart without --overwrite resumes the rest).
