...
from .attngcg_posinit_attention_utils import (
    generate_positions,
    get_nonascii_toks,
...
class AttnGCG_posinit_attention(SingleBehaviorRedTeamingMethod):
    def __init__(self,
                 target_model,
                 targets_path,
                 num_steps=50,
    ...
    def generate_test_cases_single_behavior(self, behavior_dict, verbose=False, **kwargs):
        ...
        for i in tqdm(range(num_steps)):
            ...
            behavior_optim_embeds, optim_indices = insert_optim_embed_pos(behavior_embeds, optim_embeds, optim_pos)
            ...
            cached_len = key_len - query_len
            before_len = before_embeds.shape[1]
            behavior_optim_len = behavior_optim_embeds.shape[1]
            after_len = after_embeds.shape[1]
            target_len = target_embeds.shape[1]

            if self.use_prefix_cache:
                query_target_start = behavior_optim_len + after_len

            else:
                query_target_start = before_len + behavior_optim_len + after_len

            if self.use_prefix_cache:
                key_control = optim_indices + cached_len
            else:
                key_control = optim_indices + before_len
            attn_loss = attentions[0][query_target_start-1][key_control].mean()
            ...
            embeds = [before_embeds, behavior_optim_embeds, after_embeds, target_embeds]
            loss, logit_loss, attn_loss = find_executable_batch_size(self.compute_candidates_loss, self.search_batch_size)(input_embeds, target_ids, embeds, optim_indices)
            ...
            all_logit_losses.append(logit_loss[best_idx].item())
            all_attn_losses.append(attn_loss[best_idx].item())
            all_optim_ids.append(optim_ids.tolist())

            step_times.append(time.time() - start_time)

            if (i % eval_steps == 0) or (i == num_steps - 1):
                p_output = f'\n===>Step {i}\n===>Test Case: {test_case}\n===>Loss: {current_loss}, logit_loss:{logit_loss[best_idx]}, attn_loss: {attn_loss[best_idx]}, Time: {sum(step_times):.2f}s'
            ...
            del input_embeds, sampled_top_embeds, logits, shift_logits, loss, optim_indices
            torch.cuda.empty_cache()
            gc.collect()

        logs = {'final_loss': current_loss, 'all_losses': all_losses, 'all_logit_losses': all_logit_losses, 'all_attn_losses': all_attn_losses, 'all_test_cases': all_test_cases, 'all_optim_ids' : all_optim_ids, 'optim_pos': optim_pos.tolist(), 'step_times': step_times, 'position_init_time': pos_init_time,
                'chosen_positions': chosen_positions, 'chosen_token_ids': chosen_token_ids, 'chosen_token_texts': chosen_token_texts}

        return test_case, logs

    def compute_candidates_loss(self, search_batch_size, input_embeds, target_ids, embeds, optim_indices):
        ...
        for i in range(0, input_embeds.shape[0], search_batch_size):
            ...
            before_embeds, behavior_optim_embeds, after_embeds, target_embeds = embeds
            batch_size, query_len, key_len = attentions.shape
            cached_len = key_len - query_len
            before_len = before_embeds.shape[1]
            behavior_optim_len = behavior_optim_embeds.shape[1]
            ...
            if self.use_prefix_cache:
                query_after_start = behavior_optim_len + after_len

            else:
                query_after_start = before_len + behavior_optim_len + after_len
            ...
            for b_idx in range(attentions.size(0)):
                single_attention = attentions[b_idx, query_after_start-1][key_control].mean()
