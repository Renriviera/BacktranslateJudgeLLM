...
from .gcg_hij_posinit_attention_ray_actors import (
...
class EnsembleGCG_hij_posinit_attention(RedTeamingMethod):
    use_ray = True
    ...
    def generate_test_cases(self, behaviors, verbose=False):
        ...
        for i in tqdm(range(self.num_steps)):
            ...
            loss_results = ray.get(futures)
            # 각 워커는 (behavior_losses, behavior_logit_losses, behavior_attn_losses) 튜플을 반환
            total_losses = torch.stack([result[0] for result in loss_results])
            logit_losses = torch.stack([result[1] for result in loss_results])
            attn_losses = torch.stack([result[2] for result in loss_results])

            # mean across all behaviors
            model_total_losses = total_losses.mean(1)
            model_logit_losses = logit_losses.mean(1)
            model_attn_losses = attn_losses.mean(1)

            # mean across all models
            behavior_total_losses = total_losses.mean(0)
            behavior_logit_losses = logit_losses.mean(0)
            behavior_attn_losses = attn_losses.mean(0)

            # mean across all behaviors and models
            avg_total_losses = total_losses.mean(1).mean(0)
            avg_logit_losses = logit_losses.mean(1).mean(0)
            avg_attn_losses = attn_losses.mean(1).mean(0)

            selected_id = avg_total_losses.argmin().item()
            ...
            if verbose and ((i % self.eval_steps == 0) or (i == self.num_steps - 1)):
                ...
                print(f'Total Loss: {current_total_loss:.4f} | Logit Loss: {current_logit_loss:.4f} | Attn Loss: {current_attn_loss:.4f}')
                print(f'Positions: {optim_pos.tolist()}')
                model_losses_info = [f"->Model: {self.models[idx]['model_name_or_path']} | Total Loss {model_total_losses[idx].min().item():.4f}" for idx in range(len(total_losses))]
        ...
        logs = {
            'final_loss': current_total_loss,
            'all_losses': all_losses,
            'all_logit_losses': all_logit_losses,  # 추가
            'all_attn_losses': all_attn_losses,    # 추가
