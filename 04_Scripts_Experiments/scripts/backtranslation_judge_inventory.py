...
Run: python3 04_Scripts_Experiments/scripts/backtranslation_judge_inventory.py
"""
...
def main():
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "06_Results_Artifacts/results/backtranslation_judge/design_inventory.json")
    ...
    recovery_path = "06_Results_Artifacts/results/orbits/2026-09-18-pilot/archive_strongreject.jsonl"
    recovery = {}
    for line in read(recovery_path).split("\n"):
        if line.strip():
            ...
            recovery[row["id"]] = row
    selected = json.loads(read("01_Datasets_Benchmarks/provenance/audit_selected.json"))
    pairs = json.loads(read("06_Results_Artifacts/results/fp_robustness/minimal_pairs/instantiated_pairs.json"))
    ...
    for variant, cache_relative in VARIANTS.items():
        detail_path = f"06_Results_Artifacts/results/{variant}_strongreject_olmo3_7b/details.json"
        data = json.loads(read(detail_path))
        cache = json.loads(read(f"06_Results_Artifacts/results/attacks/{cache_relative}"))
        ...
        report.append({
            ...
            "attack_cache_path": f"06_Results_Artifacts/results/attacks/{cache_relative}",
