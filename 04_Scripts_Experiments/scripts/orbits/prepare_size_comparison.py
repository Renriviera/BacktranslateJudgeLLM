...
from brass.paths import hf_hub_cache, model_snapshot, RESULTS
...
OLD = REPO / "06_Results_Artifacts/results/orbits/2026-09-18-pilot"
ROOT = REPO / "06_Results_Artifacts/results/orbits/2026-09-19-size-32b"
...
def main():
    ...
    parser.add_argument("--source-run", type=Path, default=OLD)
    parser.add_argument("--out", type=Path, required=True)
    args = parser.parse_args()
    OLD, ROOT = args.source_run.resolve(), args.out.resolve()
    if ROOT.exists() or ROOT.is_relative_to(RESULTS):
    ...
    t7 = AutoTokenizer.from_pretrained(
        model_snapshot(old_runtime["models"]["target"]["hf_id"], old_runtime["models"]["target"]["revision"]), local_files_only=True
    )
    ...
    shutil.copytree(OLD / "evaluators", ROOT / "evaluators")
    ...
    source_files = (
        list((REPO / "04_Scripts_Experiments/src/brass/orbits").glob("*.py"))
        + list((REPO / "04_Scripts_Experiments/scripts/orbits").glob("*.py"))
        + [REPO / "04_Scripts_Experiments/src/brass/serving/vllm_engine.py"]
    )
