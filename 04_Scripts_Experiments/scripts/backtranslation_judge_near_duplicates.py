...
def main():
    ...
    root = (hf_hub_cache() / "models--sentence-transformers--all-MiniLM-L6-v2")
    revision = (root / "refs/main").read_text().strip()
    torch.set_num_threads(4)
    model = SentenceTransformer(
        str(root / "snapshots" / revision), device="cpu", local_files_only=True
    )
    prompts = json.loads((repo / "06_Results_Artifacts/results/pair_strongreject_olmo3_7b/details.json").read_text())[
        "prompts"
    ]
    ...
    out = repo / "06_Results_Artifacts/results/backtranslation_judge"
