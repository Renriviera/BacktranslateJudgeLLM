...
The implemented study lives in `src/brass/backtranslation_judge/`; its CLI is
`scripts/backtranslation_judge.py`. It evaluates archived responses and never runs attack
optimization. The default run directory is `results/backtranslation_judge/2026-09-20/`.
...
```bash
python3 scripts/backtranslation_judge_inventory.py
.venv/bin/python scripts/backtranslation_judge_near_duplicates.py
.venv/bin/python scripts/backtranslation_judge.py prepare
.venv/bin/python scripts/backtranslation_judge.py export-annotations

# Validate model execution using development examples only.
PYTHONPATH=src HF_HUB_OFFLINE=1 TOKENIZERS_PARALLELISM=false \
  .venv/bin/python scripts/backtranslation_judge.py run --limit 200

# Complete the full manifest; reuse unchanged pilot events.
PYTHONPATH=src HF_HUB_OFFLINE=1 TOKENIZERS_PARALLELISM=false \
  .venv/bin/python scripts/backtranslation_judge.py run

.venv/bin/python scripts/backtranslation_judge.py analyze
.venv/bin/python scripts/backtranslation_judge_diagnostics.py
.venv/bin/python scripts/backtranslation_judge_validate_study.py
```
...
```bash
.venv/bin/python scripts/backtranslation_judge_merge_ratings.py \
...
.venv/bin/python scripts/backtranslation_judge.py validate-gold --gold /absolute/path/gold.jsonl
.venv/bin/python scripts/backtranslation_judge_calibrate.py --gold /absolute/path/gold.jsonl
.venv/bin/python scripts/backtranslation_judge.py analyze --gold /absolute/path/gold.jsonl
.venv/bin/python scripts/backtranslation_judge_validate_study.py --gold /absolute/path/gold.jsonl
```
...
PYTHONPATH=src .venv/bin/python -m pytest \
  tests/test_backtranslation_judge.py tests/test_judge_parsing.py \
  tests/test_orbits_missing_judgment.py tests/test_backtranslation_judge_utilities.py
```
...
`tests/test_backtranslation_judge_utilities.py` also checks independent-rater merging,
...
The first pilot is preserved in `results/backtranslation_judge/2026-09-20-pilot-v1/`.
