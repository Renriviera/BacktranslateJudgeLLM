...
DEFAULT_OUT = DEFAULT_PARENT / "sol_reference_codex_medium"
SOURCE_PATHS = [
...
def parse_cli(stdout, response, returncode):
    ...
    failures = [e for e in trace if e["type"] in {"error", "turn.failed"}]
...
async def evaluate(unit, out):
    ...
    try:
        ...
        if any(
            code in stdout + stderr
            for code in ["bio_policy", "content_policy_violation", "content_filter"]
        ):
            result = dict(
