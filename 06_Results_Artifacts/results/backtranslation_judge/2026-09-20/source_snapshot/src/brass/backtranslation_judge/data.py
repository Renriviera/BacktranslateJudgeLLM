...
def prepare(repo: Path, out: Path, seed=235711):
    ...
    inventory = json.loads(
        (repo / "results/backtranslation_judge/design_inventory.json").read_text()
    )
    candidates = json.loads(
        (repo / "results/backtranslation_judge/near_duplicate_candidates.json").read_text()
    )
    baseline = json.loads((repo / "results/pair_strongreject_olmo3_7b/details.json").read_text())
    ...
    recovered = {
        r["id"]: r
        for r in read_jsonl(repo / "results/orbits/2026-09-18-pilot/archive_strongreject.jsonl")
    }
    selected = json.loads((repo / ".audit_selected.json").read_text())["rows"]
    labels = {
    ...
    for variant, cache_path in VARIANTS.items():
        path = repo / f"results/{variant}_strongreject_olmo3_7b/details.json"
        source_hash = file_digest(path)
        data = json.loads(path.read_text())
        cache = json.loads((repo / "results/attacks" / cache_path).read_text())
