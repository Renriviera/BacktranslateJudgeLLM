...
def main():
    ...
    model = SentenceTransformer(model_spec["snapshot"], device="cpu", local_files_only=True)
    chunk_size = min(254, model.max_seq_length - 2)
    ...
    vectors = model.encode(
        chunks, batch_size=64, normalize_embeddings=False, show_progress_bar=True
    )
            ...
            measurements.append(
                {
                    "id": e["id"],
                    "item_id": item_id,
                    "group_id": manifest[item_id]["group_id"],
                    "cohort": e["cohort"],
                    "step": e["step"],
